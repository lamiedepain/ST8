...
def generate_fiche_docx():
    ...
    try:
        ...
        separator.space_after = Pt(6)
        
        # === TITRE ===
        titre = doc.add_paragraph('FICHE TRAVAUX')
        titre.alignment = WD_ALIGN_PARAGRAPH.CENTER
        titre.runs[0].font.bold = True
        titre.runs[0].font.size = Pt(12)
        titre.space_after = Pt(3)
        
        # Nom du chantier
        nom_chantier = doc.add_paragraph(data.get('nom', ''))
        nom_chantier.alignment = WD_ALIGN_PARAGRAPH.CENTER
        nom_chantier.runs[0].font.bold = True
        nom_chantier.runs[0].font.size = Pt(11)
        ...
        sections_data = [
            ...
            ('Référence', data.get('reference', ''), False),
            ('Adresse', f"{data.get('numero', '')} {data.get('adresse', '')}".strip(), False),
            ('Responsable', data.get('responsable', ''), False),
        ]
        
        # TYPE DE DEMANDE (conditionnelle)
        if data.get('numeroGDU') or data.get('numeroMages'):
            sections_data.extend([
                ('TYPE DE DEMANDE', '', True),
                ('N° GDU', data.get('numeroGDU', ''), False),
                ('N° Mages', data.get('numeroMages', ''), False),
                ('Date de demande', data.get('dateDemande', ''), False),
                ('Demandeur', data.get('nomDemandeur', ''), False),
            ])
        
        # DATES
        sections_data.extend([
            ('DATES ET DURÉE', '', True),
            ('Date de début', data.get('dateDebut', ''), False),
            ('Date de fin prévisionnelle', data.get('dateFin', ''), False),
            ('Nombre de jours', data.get('nombreJours', ''), False),
        ])
        
        # PRÉPARATION
        sections_data.extend([
            ('PRÉPARATION', '', True),
            ('Visite sur site', data.get('visiteSite', ''), False),
            ('Date de visite', data.get('dateVisite', ''), False),
            ("Nom de l'AM", data.get('nomAM', ''), False),
            ('Travaux envisagés', data.get('travauxEnvisages', ''), False),
            ('Priorisation', data.get('priorisationLabel', ''), False),
        ])
        
        # PRÉALABLES
        sections_data.extend([
            ('PRÉALABLES', '', True),
            ('Demande Amiante', data.get('demandeAmiante', ''), False),
            ('Présence Amiante', data.get('presenceAmiante', ''), False),
            ('N° DT/DICT', data.get('numeroDTDICT', ''), False),
            ('Date DICT', data.get('dateDICT', ''), False),
            ('Arrêté demandé le', data.get('dateDemandeArrete', ''), False),
            ("Date de l'arrêté", data.get('dateArrete', ''), False),
            ('Proximité Tram', data.get('proximiteTram', ''), False),
            ('Protocole signé', data.get('protocoleSign', ''), False),
        ])
        
        # DESCRIPTION (conditionnelle)
        if data.get('description'):
            sections_data.append(('Description', data.get('description', ''), False))
        
        # RESSOURCES
        sections_data.extend([
            ('RESSOURCES', '', True),
            ("Nombre d'agents", data.get('nombreAgents', ''), False),
        ...
        for i, (label, value, is_header) in enumerate(sections_data):
            ...
            if is_header:
                # Fusionner les cellules pour les en-têtes de section
                cell_merged = cell_label.merge(cell_value)
                cell_merged.text = label
        ...
        # === AGENTS, VÉHICULES, MATÉRIAUX (sous forme de texte simple) ===
        if data.get('agents') or data.get('vehicules') or data.get('articles') or data.get('outils'):
            ...
            if data.get('agents'):
                details_text.append(f"Agents: {', '.join(data.get('agents', []))}")
            
            if data.get('vehicules'):
                details_text.append(f"Véhicules: {', '.join(data.get('vehicules', []))}")
            
            if data.get('articles'):
                details_text.append(f"Matériaux: {', '.join(data.get('articles', []))}")
